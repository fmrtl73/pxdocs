...
## Security tokens
To enable Portworx security the administrator must generate a token using their own TA application. The administrators can also generate user tokens using `pxctl` command-line tool. For more details, refer to [Generate token](/reference/cli/generate-token).
...
To enhance the security of your cluster, enable PX-Security in Portworx. For more details, refer to the [Deploying in Kubernetes](/cloud-references/security/kubernetes/) page.
